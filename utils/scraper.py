...
async def scrape_urls(urls: list[str]) -> dict[str, Optional[str]]:
    ...
    async with httpx.AsyncClient(timeout=15, follow_redirects=True) as session:
        ...
        for url, html in zip(urls, html_pages):
            if html:
                results[url] = extract_text(html)  # Limit to 500 chars
...
async def use_scraper(search_results: list[dict], st: float) -> str:
    ...
    print(f"📋 Found {len(urls)} pages in {st}s:")
    ...
    print(
        f"✅ Successfully scraped {successful_scrapes}/{len(urls)} pages in {t2 - t1}s"
    )
    return folder_name
